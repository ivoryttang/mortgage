//@ts-nocheck
"use client"
import Navbar from "../ui/Navbar"
import { useEffect } from "react"
export default function Page() {

    return <div className="p-20 md:flex-row flex-col"><nav style={{ marginBottom: '100px' }}><Navbar /></nav>
    <div style={{ display: 'flex', justifyContent: 'center' }}>
    <div className="text-center mr-10 w-[800px]" ><h2>Meet Reggie</h2>
    Deliver compliant, reliable, and cost-effective lending solutions
    </div>
    <div>
    Engaged qualified workforce in the form of multidisciplinary experts, strategic advisors, and offshore teams to enable 24/7 service to handle the increase in work volume.
<br></br>
<br></br>
Redefined KPIs and generate compliance reports with the help of AI.
<br></br>
<br></br>
Delegated a special workforce that can specifically focus on key areas with strict regulatory requirements and high chance of potential frauds.
<br></br>
<br></br>
Simulated stress scenarios to help you assess your resilience, identify business risks, and accordingly frame mortgage forbearance, refinance, and risk management measures.
<br></br>
<br></br>
Adapted our service delivery with your preferred technology to process a greater number of applications and preserve data integrity.
    </div>
    </div>
    </div>
  }